...
class ModelEvaluator:
    ...
    - Not worse than baseline by more than 0.01
    """
    ...
    def check_promotion_guardrails(self, baseline_report: Dict, candidate_report: Dict) -> Tuple[bool, str]:
        ...
        # Guardrail 3: Not worse than baseline
        max_degradation = self.promotion_rules['max_degradation']
        auc_diff = candidate_metrics['auc'] - baseline_metrics['auc']
        if auc_diff < -max_degradation:
            reason = f"Worse than baseline: AUC difference {auc_diff:.4f} < -{max_degradation}"
            logger.warning(f"❌ Guardrail 3 FAILED: {reason}")
            return False, reason
        logger.info(f"✅ Guardrail 3 PASSED: AUC difference {auc_diff:+.4f} >= -{max_degradation}")
    ...
    def _save_markdown_report(self, filepath: Path, comparison: Dict, should_promote: bool, reason: str):
        ...
        with open(filepath, 'w') as f:
            ...
            f.write(f"3. Maximum Degradation: {self.promotion_rules['max_degradation']}\n\n")
